...
def mint(**kwargs):
    ...
    if values[0] is None:
        raise ValueError(
            f"Plugin '{issuer.get_title()}' provided no root certification. Check plugin configuration."
        )
...
def create_authority_roles(roles, owner, plugin_title, creator):
    ...
    for r in roles:
        ...
        if role:
            raise Exception(
                "Unable to create authority role {} because it already exists".format(r["name"])
            )
