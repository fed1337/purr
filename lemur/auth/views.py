...
def exchange_for_access_token(
    ...
    if not r.ok or "error" in response:
        raise TokenExchangeFailed(
            response.get("error", "Unknown error"), response.get("error_description", "")
        )
...
def create_user_roles(profile: dict) -> list[str]:
    ...
    for idp_groups_key in idp_groups_keys:
        ...
        if not isinstance(profile[idp_groups_key], list) and all(
            isinstance(item, str) for item in profile[idp_groups_key]
        ):
        ...
        matched_roles = [
            idp_group_to_role_map[role]
            for role in profile.get(idp_groups_key, [])
            if role in idp_group_to_role_map
        ]
        ...
        if current_app.config.get("IDP_ASSIGN_ROLES_FROM_USER_GROUPS", True):
            idp_roles_description = current_app.config.get(
                "IDP_ROLES_DESCRIPTION",
            ...
            for group in profile.get(idp_groups_key, []):
                if group in ["admin", "operator", "read-only"] and current_app.config.get(
                ...
                if group.startswith(idp_roles_prefix) and group.endswith(idp_roles_suffix):
                    ...
                    if not role and current_app.config.get(
