...
def test_format_nonexpiration(certificate, endpoint):
    ...
    for certificate in data:
        expected_message = {
            ...
            "details": "https://lemur.example.com/#/certificates/{name}".format(
...
def create_and_subscribe_to_topic():
    ...
    queue_arn = sqs_client.get_queue_attributes(QueueUrl=queue_url, AttributeNames=["All"])[
        "Attributes"
    ]["QueueArn"]
    ...
    for certificate in data:
        ...
        actual_message = next(
            (
                ...
                if json.loads(m["Body"])["MessageId"] == expected_message_id
            ),
            None,
        )
...
def prepare_test():
    ...
    for prior_notification in service.get_all():
        service.update(
            prior_notification.id,
            prior_notification.label,
            prior_notification.plugin_name,
            prior_notification.options,
            prior_notification.description,
            False,
            [],
            [],
        )
