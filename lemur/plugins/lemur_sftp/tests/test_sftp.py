...
class TestSftp(unittest.TestCase):
    ...
    def test_upload_file_single_with_password(self, mock_paramiko):
        ...
        mock_ssh.connect.assert_called_with(
            "non-existent", username="test_acme", port="22", password="test_password"
        )
    ...
    def test_upload_file_multiple_with_key(self, mock_paramiko):
        ...
        options = [
            ...
            {"name": "port", "value": "22"},
            {"name": "user", "value": "test_acme"},
            {"name": "privateKeyPath", "value": "/var/id_rsa"},
        ...
        mock_paramiko.RSAKey.from_private_key_file.assert_called_with(
            "/var/id_rsa", "ssh-key-password"
        )
        mock_ssh.connect.assert_called_with(
            "non-existent", username="test_acme", port="22", pkey="ssh-rsa test-key"
        )
