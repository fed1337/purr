...
def test_determine_end_date(mock_current_app):
    with freeze_time(time_to_freeze=arrow.get(2016, 11, 3).datetime):
        assert arrow.get(2017, 12, 3).format("YYYY-MM-DD") == plugin.determine_end_date(
            0
        )  # 1 year + 1 month
        assert arrow.get(2017, 3, 5).format("YYYY-MM-DD") == plugin.determine_end_date(
            arrow.get(2017, 3, 5)
        )
        assert arrow.get(2017, 12, 3).format("YYYY-MM-DD") == plugin.determine_end_date(
            arrow.get(2020, 5, 7)
...
def test_process_options_infer_eku(mock_load_pem, mock_current_app, authority):
    ...
    mock_csr.extensions = Extensions(
        [
            Extension(
                oid=x509.oid.ExtensionOID.EXTENDED_KEY_USAGE, critical=False, value=both_eku_ext
            )
        ]
    )
    ...
    mock_csr.extensions = Extensions(
        [
            Extension(
                oid=x509.oid.ExtensionOID.EXTENDED_KEY_USAGE, critical=False, value=client_eku_ext
            )
        ]
    )
    ...
    mock_csr.extensions = Extensions(
        [
            Extension(
                oid=x509.oid.ExtensionOID.EXTENDED_KEY_USAGE, critical=False, value=server_eku_ext
            )
        ]
    )
...
def test_create_authority(app):
    ...
    options = {"name": "test Entrust authority"}
    p = plugins.get("entrust-issuer")
    entrust_root, intermediate, role = p.create_authority(options)
    assert role == [
        {"username": "", "password": "", "name": "entrust_test_Entrust_authority_admin"}
    ]
