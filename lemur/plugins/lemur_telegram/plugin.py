...
def create_certificate_url(name):
    """
    Return URL for certificate details page
    """
...
def create_expiration_attachments(certificates):
    """
    Prepare the telegram message body for expiration notifications
    """
    attachments = []
...
def create_rotation_attachments(certificate):
    """
    Prepare the telegram message body for rotation notifications
    """
    name = escape(certificate["name"])
    owner = escape(certificate["owner"])
    url = create_certificate_url(certificate["name"])
...
class TelegramNotificationPlugin(ExpirationNotificationPlugin):
    ...
    def send(self, notification_type, message, targets, options, **kwargs):
        ...
        `lemur notify`
        While `targets` parameter received, it is unused; the plugin currently supports sending to only one chat.
        """
        attachments = None
