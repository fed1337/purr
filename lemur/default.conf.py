...
IDP_GROUPS_KEYS = [
    "googleGroups"
]  # a list of keys used by IDP(s) to return user groups (profile[IDP_GROUPS_KEY])
IDP_ASSIGN_ROLES_FROM_USER_GROUPS = (
    True  # Assigns a Lemur role for each group found attached to the user
)
IDP_CREATE_ROLES_FROM_USER_GROUPS = (
    True  # Creates a Lemur role for each group found attached to the user if missing
)
...
IDP_CREATE_PER_USER_ROLE = (
    True  # Generates Lemur role for each user (allows cert assignment to a single user)
)
