...
def report_celery_last_success_metrics():
    ...
    red.set(f"{function}.last_success", int(time.time()))  # Alert if this metric is not seen
...
def fetch_acme_cert(id, notify_reissue_cert_id=None):
    ...
    for cert in resolved_certs:
        ...
        if real_cert:
            ...
            pending_certificate_service.update(cert.get("pending_cert").id, resolved=True)
